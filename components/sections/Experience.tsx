...
export default function Experience() {
  ...
  return (
    <section id="experience" className="h-full flex flex-col px-8 md:px-16">
      <div className="shrink-0 flex items-center gap-4 mb-7 pt-6 md:pt-0">
        <span className="font-mono text-sm text-accent">03</span>
        <span className="w-11 h-px bg-white/20" />
        <h2 className="text-xl md:text-3xl font-semibold">Experience</h2>
      </div>

      <div className="flex-1 overflow-y-auto no-scrollbar">
        <div className="relative pl-10 flex flex-col gap-8 pb-6">
          ...
          {experience.map((x) => (
            <div key={x.id} className="relative">
              ...
              <p className="max-w-xl text-sm text-text-muted mt-1.5">
                {x.blurb}
              </p>
            </div>
          ))}
