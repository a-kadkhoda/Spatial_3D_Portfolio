...
- **Deck mode (desktop)** — real 3D travel. All seven screens are parked in one `transform-style: preserve-3d` stage at staggered X/Y offsets and `-index * 1500px` on Z (`SCREEN_OFFSETS`, `screenTransform`). Navigation moves *the stage*, not the screens (`stageOffset`), so the browser composites a single transform. A GSAP tween drives that stage with `DECK_EASE`, while each screen tweens `autoAlpha` so neighbours sit at 0.04/0.02 opacity and read as distant objects. Body scroll is locked.

  **`DEPTH_STEP` equals the perspective (1500px), so the screen one index *below* the active one is parked exactly at the eye.** Travel drags it across the perspective origin, where its scale runs to infinity — a full-viewport layer, text included, re-rasterised at a new scale every frame. That cost is why reverse navigation used to collapse to ~20fps. Two rules keep it away, both in `Deck.tsx`: the blur is `gsap.set`, never tweened (animating a filter on a layer that is simultaneously scaling is what actually burns the frame budget, not the transform), and the `autoAlpha` timing is direction-aware — screens crossing toward the camera drop out in `EXIT_DURATION`, screens returning from it wait `RETURN_DELAY` before fading back in, so neither is painted while it is blown up. Those tweens use `overwrite: true`, not `"auto"`: a delayed tween has not rendered yet, so an auto-overwriting successor would leave it queued and let it win after the fact.
